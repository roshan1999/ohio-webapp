...
function addMapFiltersBarLine() {
  ...
  let innerHTML = ` Number of records<input type="number" name="barline_records" value="2000" id="barline_input">`
...
function plot_graph(x_axis_data, parentDiv) {
    ...
    const data = Object.entries(line_data).map(([key, value]) => ({ key, average: value.average }));

    const bar_data_process = Object.entries(bar_data).map(([key, value]) => ({ key, average: value.average_rate }));
